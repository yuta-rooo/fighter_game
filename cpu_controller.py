...
class CPUController:
    ...
    def __post_init__(self) -> None:
        if self.difficulty not in DIFFICULTY_SETTINGS:
            raise ValueError(
                f"Unknown CPU difficulty: {self.difficulty}"
            )
    ...
    def decide(self, game: Any) -> dict[str, bool]:
        ...
        if human_is_threatening and cpu.on_ground:
            if (
                self.rng.random()
                < float(self.settings["guard_chance"])
            ):
                inp["guard"] = True
            ...
            if (
                self.rng.random()
                < float(self.settings["jump_chance"]) * 2.0
            ):
                inp["jump"] = True
                self.hold_input = inp.copy()
                return inp
        ...
        if distance > 118:
            ...
            if (
                cpu.on_ground
                and self.rng.random()
                < float(self.settings["jump_chance"])
            ):
                inp["jump"] = True
        ...
        if (
            distance < 58
            and self.rng.random()
            < float(self.settings["retreat_chance"])
        ):
            if human.x < cpu.x:
                inp["right"] = True
            else:
        ...
        if (
            cpu.meter >= 30
            and distance <= 145
            and roll < float(self.settings["special_chance"])
        ):
            inp["special"] = True
            ...
            distance <= 105
            and roll
            < float(self.settings["special_chance"])
            + float(self.settings["heavy_chance"])
        ):
            inp["heavy"] = True
